...
def authorize_device(device_id: str, x_runner_token: str = Header(default=""), db: Session = Depends(get_db)):
    try: return authorize_runner(db, device_id, x_runner_token)
    except PermissionError as exc: raise HTTPException(401, str(exc))
@app.get("/healthz")
...
def intake(payload: TaskCreate, db: Session = Depends(get_db)):
    try:
        ...
        if decision["kind"] == "chat": return {"kind":"chat", "reply":chat_reply(payload.text), "reason":decision.get("reason", "conversation")}
    ...
    except LookupError as exc: raise HTTPException(409, str(exc))
    except LLMUnavailable as exc: raise HTTPException(503, str(exc))
    except Exception as exc: raise HTTPException(422, f"Unable to interpret request: {type(exc).__name__}: {exc}")
@app.post("/v1/tasks", dependencies=[Depends(authorize_bot)])
def submit(payload: TaskCreate, db: Session = Depends(get_db)):
    try: return serialize(create_task(db,payload.requester_id,payload.text,payload.device_id))
    except LookupError as exc: raise HTTPException(409,str(exc))
@app.get("/v1/tasks", dependencies=[Depends(authorize_bot)])
def tasks(limit:int=10, db:Session=Depends(get_db)): return [serialize(x) for x in db.query(Task).order_by(Task.created_at.desc()).limit(min(limit,100)).all()]
...
def approve(task_id:str, requester_id:str, db:Session=Depends(get_db)):
    item=db.get(Task,task_id)
    if not item: raise HTTPException(404,"task not found")
    if item.requester_id != requester_id: raise HTTPException(403,"requester mismatch")
    if not item.requires_approval: raise HTTPException(409,"approval not required")
    if item.approval_expires_at and item.approval_expires_at.timestamp() < __import__('time').time(): item.status="FAILED_POLICY"; db.commit(); raise HTTPException(409,"approval expired")
    item.approved=True; item.status="QUEUED"; audit(db,item.id,"TASK_APPROVED","Approved through Telegram"); db.commit(); return serialize(item)
...
def cancel(task_id:str, requester_id:str, db:Session=Depends(get_db)):
    ...
    if not item: raise HTTPException(404,"task not found")
    if item.requester_id != requester_id: raise HTTPException(403,"requester mismatch")
    if item.status in {"SUCCEEDED","FAILED","CANCELLED","FAILED_POLICY"}: raise HTTPException(409,"task is terminal")
    item.status="CANCELLED"; audit(db,item.id,"TASK_CANCELLED","Cancelled through Telegram"); db.commit(); return serialize(item)
@app.post("/v1/runners/register")
def register(payload:DeviceRegister,db:Session=Depends(get_db)):
    try:
        d=register_runner(db,payload); return {"id":d.id,"status":d.status}
    except PermissionError as exc: raise HTTPException(401,str(exc))
@app.post("/v1/runners/{device_id}/heartbeat")
def heartbeat(device_id:str,payload:DeviceHeartbeat,db:Session=Depends(get_db)):
    try:
        d=authorize_runner(db,device_id,payload.token); d.status=payload.status; db.commit(); return {"status":"ok"}
    except PermissionError as exc: raise HTTPException(401,str(exc))
@app.get("/v1/runners/{device_id}/next")
def lease(device_id:str,_:Device=Depends(authorize_device),db:Session=Depends(get_db)):
    item=next_task(db,device_id); return {"task":serialize(item) if item else None}
...
def update(device_id:str,task_id:str,payload:TaskUpdate,_:Device=Depends(authorize_device),db:Session=Depends(get_db)):
    item=db.get(Task,task_id)
    if not item or item.device_id != device_id: raise HTTPException(404,"task not found")
    item.status=payload.status
    if payload.plan is not None: item.plan_json=json.dumps(payload.plan)
    if payload.result is not None: item.result_json=json.dumps(payload.result)
    if payload.error is not None: item.error=payload.error
    audit(db,item.id,"TASK_"+payload.status,payload.error or "runner update"); db.commit(); return serialize(item)
